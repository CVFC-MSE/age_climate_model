mse.simulation <- function(pars, years = 100, sims = 1000, m.maturity = c(0.038, 0.5, 0.999, 1), 
                           n.surv = c(0.5, 0.8, 0.8, 0.8), scenario = NULL){
  ...
  mse.out <- foreach(sim = 1:n.sim) %dopar% {
    ...
    for(t in 2:n.yr){
      ...
      SI.forecast[t, ] <- sum(N[c(3:5,11:13), t - 1, ], H[c(3:5,11:13), t - 1, ]) * exp(forecast.error)
      ...
      # tmp <- rbeta(1000, c.alpha, c.beta)
      # ggplot() +
      #   geom_histogram(aes(tmp)) +
      #   geom_vline(xintercept = 0.7, color = 'red') +
      #   geom_vline(xintercept = mean(tmp), color = 'blue') +
      #   geom_vline(xintercept = median(tmp), color = 'blue', lty = 'dashed')
      # ggplot() + geom_histogram(aes(x=tmp)) +
      #   geom_vline(xintercept = mean(tmp), color = 'red') +
      #   geom_vline(xintercept = median(tmp), color = 'red', lty = 'dashed') +
      #   scale_x_continuous(limits = c(0, 1))

      #Fishery impact
      age.c[, t, ] <- real.c[t, ] * nu
      I.H[N.H.O.ind, t, ] <- rbinom(rep(1, length(N.H.O.ind)), H[N.H.O.ind, t - 1, ], age.c[, t, ])
      I.N[N.H.O.ind, t, ] <- rbinom(rep(1, length(N.H.O.ind)), N[N.H.O.ind, t - 1, ], age.c[, t, ])
      ...
      H[c(2, 2*A), t, ] <- rmultinom(1, H[1, t, ], c(0.5 * n[1, t, ] * phi.2 * hd[t], 0.5 * n[1, t, ] * phi.2 * hd[t], 1 - n[1, t, ] * phi.2 * hd[t]))[1:2]
      N[c(2, 2*A), t, ] <- rmultinom(1, N[1, t, ], c(0.5 * n[1, t, ] * phi.2, 0.5 * n[1, t, ] * phi.2, 1 - (n[1, t, ] * phi.2)))[1:2]
      ...
      for(a in 2:A){
        if(a < A){
          ...
          # female
          H[c(f.o, f.s), t, ] <- rmultinom(1, H[f.i, t - 1, ] - I.H[f.i, t, ], c(n[a, t, ] * (1 - m.hat[2, a - 1]), n[a, t, ] * m.hat[2, a - 1], 1 - n[a, t, ]))[1:2]
          N[c(f.o, f.s), t, ] <- rmultinom(1, N[f.i, t - 1, ] - I.N[f.i, t, ], c(n[a, t, ] * (1 - m.nat[2, a - 1]), n[a, t, ] * m.nat[2, a - 1], 1 - n[a, t, ]))[1:2]
          # male
          H[c(m.o, m.s), t, ] <- rmultinom(1, H[a, t - 1, ] - I.H[a, t, ], c(n[a, t, ] * (1 - m.hat[1, a - 1]), n[a, t, ] * m.hat[1, a - 1], 1 - n[a, t, ]))[1:2]
          N[c(m.o, m.s), t, ] <- rmultinom(1, N[a, t - 1, ] - I.N[a, t, ], c(n[a, t, ] * (1 - m.nat[1, a - 1]), n[a, t, ] * m.nat[1, a - 1], 1 - n[a, t, ]))[1:2]
        } else {
          # female
          H[n.age.stage, t, ] <- rbinom(1, H[n.age.stage - A + 1, t - 1, ] - I.H[n.age.stage - A + 1, t, ], n[a, t, ])
          N[n.age.stage, t, ] <- rbinom(1, N[n.age.stage - A + 1, t - 1, ] - I.N[n.age.stage - A + 1, t, ], n[a, t, ])
          # male
          H[2*A - 1, t, ] <- rbinom(1, H[A, t - 1, ] - I.H[A, t, ], n[a, t, ])
          N[2*A - 1, t, ] <- rbinom(1, N[A, t - 1, ] - I.N[A, t, ], n[a, t, ])
        }
      }
